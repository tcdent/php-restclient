...
class RestClient implements Iterator, ArrayAccess {
    ...
    public $rawresponse; // Full response headers + body
    ...
    public function execute($url, $method = 'GET', $parameters = [], $headers = []) {
        ...
        $client->curloptions = $curlopt;

        $client->rawresponse = curl_exec($client->handle);
        $client->parse_response($client->rawresponse);
    ...
    public function parse_response($response) {
        ...
        $checkForNewHeader = false;
        do {
            if ($checkForNewHeader) {
                $checkForNewHeader = false;
                if (count($headers) > 0 && strpos($line, 'HTTP') !== 0)
                    break;
                else {
            ...
            if (strlen(trim($line)) == 0) {
                ...
                if (count($headers) > 0) {
                    $checkForNewHeader = true; // Must be the newline after headers, move on to response body
                }
        ...
        $this->history_headers = $historyHeaders;
        $this->headers = (object) $headers;
        $this->response = $line . strtok("");
    }
